...
const App = () => {
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', padding: '25px', textAlign: 'center' }}>
      <img src="/icons/logo_aimentary.png" alt="Logo A.I.mentary" style={{ width: 120, marginBottom: '15px' }} />
      <h1 style={{ color: '#4CAF50' }}>A.I.mentary</h1>
      {!formData.registrato ? (
        <div>
          <input type="text" name="nome" placeholder="Nome" onChange={handleChange} style={{ margin: '10px' }} /><br />
          <select name="sesso" onChange={handleChange} style={{ margin: '10px' }}>
          ...
          <select name="giorno" onChange={handleChange} style={{ margin: '4px' }}>{giorni.map(d => <option key={d}>{d}</option>)}</select>
          <select name="mese" onChange={handleChange} style={{ margin: '4px' }}>{mesi.map(m => <option key={m}>{m}</option>)}</select>
          <select name="anno" onChange={handleChange} style={{ margin: '4px' }}>{anni.map(a => <option key={a}>{a}</option>)}</select><br />
